...
class ArquiteturaX86{
  ...
  ArquiteturaX86(RegGerais gerais, RegSeletorSegmentos &seletores_segmento, RegOffsets &offset, map <long, string> memoria, TabelaDescritorSegmento &tabela, RegFlag &flag)
    : gerais(gerais), seletores_segmento(seletores_segmento), offset(offset), flag(flag), tabela(tabela), memoria(memoria) {
...
void ArquiteturaX86::cmp(Endereco<32>& END1, Endereco<32>& END2) {
  ...
  //OF = (sinalOperando1 == sinalOperando2) && (sinalResultado != sinalOperando1)
  if ((sinalOperando1 == sinalOperando2) && (sinalResultado != sinalOperando1)) {
  ...
  else 
  {
    ...
    cout << "Resultado da comparação (valorReg1 - valorReg2): " << resultado << endl;
